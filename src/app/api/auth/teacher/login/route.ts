...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = await loginByRole(body.identifier, body.password, "TEACHER");
    if (!user) return jsonError("Invalid teacher credentials", 401);

    const payload = await buildSessionPayload(user);
    const token = await createToken(payload);
    await setRoleSessionCookie("TEACHER", token);
